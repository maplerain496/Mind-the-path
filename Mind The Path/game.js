...
const SENTENCES = [
    ...
    {
        id: 10,
        words: ['The', 'ship', 'floated', 'past', 'the', 'harbor', 'sank'],
        display: 'The ship floated past the harbor sank.',
        translation: '那艘漂过港口的船沉没了。',
        structure: {
            NP: { position: 0, word: 'The ship', words: [0, 1] },
            Participial: { position: 1, word: 'floated past the harbor', words: [2, 3, 4, 5] },
            VP: { position: 2, word: 'sank', words: [6] }
        },
        slots: [
            { id: 'np', label: 'NP (主语)', type: 'NP', required: true },
            { id: 'participial', label: 'Participial (分词短语)', type: 'Participial', required: true },
        ...
        isGardenPath: true,
        trapWord: 'floated',
        trapExplanation: '"floated"被误认为是主句谓语，但实际上是修饰"ship"的后置定语！',
        difficulty: 2
    },
    {
        id: 11,
    ...
    {
        id: 32,
        words: ['The', 'government', 'plans', 'failed'],
        display: 'The government plans failed.',
        translation: '政府的计划失败了。',
        structure: {
            NP: { position: 0, word: 'The government', words: [0, 1] },
            VP: { position: 1, word: 'plans', words: [2] },
            Result: { position: 2, word: 'failed', words: [3] }
        },
        slots: [
            { id: 'np', label: 'NP (主语)', type: 'NP', required: true },
            { id: 'vp', label: 'VP (谓语)', type: 'VP', required: true },
            { id: 'result', label: 'Result (结果)', type: 'Result', required: true }
        ],
        isGardenPath: true,
        trapWord: 'plans',
        trapExplanation: '"plans"看起来像名词（计划），但实际上是动词（计划/规划）！',
        difficulty: 3
    },
    ...
    {
        id: 42,
        words: ['The', 'clear', 'clears', 'the', 'way'],
        display: 'The clear clears the way.',
        translation: '清澈的东西扫清道路。',
        structure: {
            NP: { position: 0, word: 'The clear', words: [0, 1] },
            VP: { position: 1, word: 'clears', words: [2] },
            NP2: { position: 2, word: 'the way', words: [3, 4] }
        },
        slots: [
            { id: 'np', label: 'NP (主语)', type: 'NP', required: true },
            { id: 'vp', label: 'VP (谓语)', type: 'VP', required: true },
        ...
        isGardenPath: true,
        trapWord: 'clears',
        trapExplanation: '"clear clears"看起来是重复，但第一个是名词（清澈），第二个是动词（扫清）！',
        difficulty: 3
    },
...
function loadLevel() {
    ...
    // 前3关：2条普通句 + 1条简单陷阱句（花园路径句占约33%）
    if (level <= 3) {
        // 混合简单句和简单花园路径句
        const simpleSentences = SENTENCES.filter(s => s.difficulty <= 2);
        availableSentences = simpleSentences.filter(s => !gameState.usedSentenceIds.includes(s.id));
    } else {
        // 后续关卡：花园路径句占比80%以上
        const gardenPathSentences = SENTENCES.filter(s => s.isGardenPath && s.difficulty <= Math.ceil(level / 2));
        const normalSentences = SENTENCES.filter(s => !s.isGardenPath && s.difficulty <= Math.ceil(level / 2));
        
        // 80%花园路径句 + 20%普通句
        const selectedGardenPath = gardenPathSentences.filter(s => !gameState.usedSentenceIds.includes(s.id));
        const selectedNormal = normalSentences.filter(s => !gameState.usedSentenceIds.includes(s.id));
        
        // 随机选择：80%概率选花园路径句
        if (selectedGardenPath.length > 0 && (Math.random() < 0.8 || selectedNormal.length === 0)) {
            availableSentences = selectedGardenPath;
        } else {
            availableSentences = selectedNormal.length > 0 ? selectedNormal : selectedGardenPath;
        }
    }
    ...
    if (availableSentences.length === 0) {
        ...
        availableSentences = SENTENCES.filter(s => s.difficulty <= Math.ceil(level / 2));
    }
...
function onDrop(e) {
    ...
    if (wordElement && !wordElement.classList.contains('placed')) {
        placeWordInSlot(wordElement, e.target);
    }
}
...
function placeWordInSlot(wordElement, slotElement) {
    ...
    slotElement.innerHTML = `<span class="slot-content">${word}</span>`;
...
function checkSolution() {
    ...
    for (const [slotType, slotInfo] of Object.entries(structure)) {
        ...
        const placedWord = gameState.placedWords.find(pw => pw.slotId === (slotElement.dataset.slotId));
        ...
        if (!placedWord) {
            ...
            // 检查单词是否正确
            const expectedWords = slotInfo.words.map(i => sentence.words[i]).join(' ');
            const actualWord = placedWord.word;
            
            // 简化检查：只检查第一个单词
            const expectedFirstWord = sentence.words[slotInfo.words[0]];
            
            if (actualWord !== expectedFirstWord) {
                isCorrect = false;
...
function gameOver() {
    ...
    elements.overlayDesc.innerHTML = `
        <p>时间到！</p>
        <p><strong>最终得分:</strong> ${gameState.score}</p>
        <p><strong>完成关卡:</strong> ${gameState.level - 1}</p>
        <br>
        <p>你成功避开了多少次花园路径陷阱？</p>
    `;
